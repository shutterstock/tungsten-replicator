module ConfigureDeploymentStepDeployment
  ...
  def build_home_directory
    ...
    mkdir_if_absent("#{@config.getProperty(HOME_DIRECTORY)}/releases")
  ...
  def create_release
    if @config.getProperty(DEPLOY_CURRENT_PACKAGE) == "true"
      debug("Copy #{Configurator.instance.get_base_path()} to #{@config.getProperty(HOME_DIRECTORY)}/releases")
      cmd_result("cp -rf #{Configurator.instance.get_base_path()} #{@config.getProperty(HOME_DIRECTORY)}/releases")
      ...
      if uri.scheme == "http" || uri.scheme == "https"
        ...
        if package_basename =~ /.tar.gz$/
          ...
          cmd_result("cd #{@config.getProperty(HOME_DIRECTORY)}/releases; tar xf #{@config.getProperty(TEMP_DIRECTORY)}/#{package_basename}.tar")
        elsif File.directory?("#{@config.getProperty(TEMP_DIRECTORY)}/#{package_basename}")
          cmd_result("cp -rf #{@config.getProperty(TEMP_DIRECTORY)}/#{package_basename} #{@config.getProperty(HOME_DIRECTORY)}/releases")
        else
    ...
    debug("Create symlink to #{@config.getProperty(HOME_DIRECTORY)}/releases/#{package_basename}")
    cmd_result("rm -f #{get_deployment_basedir()}; ln -s #{@config.getProperty(HOME_DIRECTORY)}/releases/#{package_basename} #{get_deployment_basedir()}")
  ...
  def deploy_config_files
    debug("Copy #{Configurator.instance.get_config_filename()} to #{@config.getProperty(HOME_DIRECTORY)}/configs/#{Configurator::HOST_CONFIG}")
    FileUtils.cp("#{Configurator.instance.get_config_filename()}", "#{@config.getProperty(HOME_DIRECTORY)}/configs/#{Configurator::HOST_CONFIG}", :preserve => true)
