...
class WriteableTempDirectoryCheck < ConfigureValidationCheck
  ...
  def validate
    # The -w flag will tell us if the directory is writeable
    ...
    unless writeable == "0"
      error "Unable to write to #{@config.getProperty(GLOBAL_TEMP_DIRECTORY)}"
    else
      debug "#{@config.getProperty(GLOBAL_TEMP_DIRECTORY)} directory is writeable"
    end
  end
end
...
class WriteableHomeDirectoryCheck < ConfigureValidationCheck
  ...
  def validate
    ...
    writeable = cmd_result("if [ -w #{@config.getProperty(GLOBAL_HOME_DIRECTORY)} ]; then echo 0; else echo 1; fi")
    
    unless writeable == "0"
      error "Unable to write to #{@config.getProperty(GLOBAL_HOME_DIRECTORY)}"
    else
      debug "#{@config.getProperty(GLOBAL_HOME_DIRECTORY)} directory is writeable"
    end
  end
end
